...
void interpolation(void)
{
	...
	KeyFrame[playIndex].rotIncCodIzq = (KeyFrame[playIndex + 1].rotCodIzq - KeyFrame[playIndex].rotCodIzq) / i_max_steps;
	KeyFrame[playIndex].rotIncCodDer = (KeyFrame[playIndex + 1].rotCodDer - KeyFrame[playIndex].rotCodDer) / i_max_steps;
...
void animate(void)
{
	if (play)
	{
		...
		else
		{
			...
			rotCodIzq += KeyFrame[playIndex].rotIncCodIzq;
			rotCodDer += KeyFrame[playIndex].rotIncCodDer;
	...
	Model brazoDer, Model brazoIzq, Model cabeza, Model piso)
{
	...
	model = glm::rotate(model, glm::radians(-rotCodDer), glm::vec3(0.0f, 0.0f, 1.0f));
	shader.setMat4("model", model);
	...
	model = glm::rotate(model, glm::radians(0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	model = glm::translate(model, glm::vec3(0.75f, 2.5f, 0));
	model = glm::rotate(model, glm::radians(-rotCodIzq), glm::vec3(0.0f, 0.0f, 1.0f));
	shader.setMat4("model", model);
...
int main()
{
	...
	for (int i = 0; i < MAX_FRAMES; i++)
	{
		...
		KeyFrame[i].rotIncCodIzq = 0;
		KeyFrame[i].rotCodDer = 0;
		KeyFrame[i].rotIncCodDer = 0;
	}
