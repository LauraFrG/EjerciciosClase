...
void display(void)
{
	...
	//Rojo
	//model = glm::translate(model/*model = glm::mat4(1.0f)*/, glm::vec3(5, 0, 0)); //No es necesario inicializar, ya que aún no modificamos la matriz
/*	modelTemp = model;
	...
	//Azul
	model = glm::translate(model = glm::mat4(1.0f), glm::vec3(-10, 0, 0));
	model = glm::scale(model, glm::vec3(8, 8, 8));
	projectionShader.setVec3("aColor", glm::vec3(0.0f, 0.0f, 1.0f));
	projectionShader.setMat4("model", model);
	glDrawArrays(GL_QUADS, 0, 24);
